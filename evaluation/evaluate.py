...
def main():
    parser = argparse.ArgumentParser(description="Calculate GrabCut metrics on OxfordPets val split")
    ...
    image_files = [f for f in os.listdir("outputs/classical") if f.endswith('.png')]
    preds_masks = []

    for file in image_files:
        img_path = os.path.join("classical", file)
        img = cv2.imread(img_path, cv2.IMREAD_GRAYSCALE)
        bin_mask = (img > 0).astype(np.uint8)
        preds_masks.append(bin_mask)

    dataset = OxfordPetsSegmentation(root=args.dataset_root, split="val", image_size=128)
    ...
    if args.all:
        for i in tqdm(range(len(dataset)), desc="GrabCut"):
            ...
            iou = compute_iou(preds_masks[i], mask)
            accuracy = pixel_accuracy(preds_masks[i], mask)
        ...
        mask = mask_t.numpy().squeeze().astype(bool)
        iou = compute_iou(preds_masks[args.index], mask)
        accuracy = pixel_accuracy(preds_masks[args.index], mask)
        classical_acuracy.append(accuracy)
        classical_iou.append(iou)
